$(document).ready(function () {
        ...
        return {
          results: $.map(data, function (item) {
            ...
            return {
              ...
              id: item.product_id,
              image: checkdefault(
                "data/product_icon/default/product.png",
  ...
  $("#submit_button").on("click", function (e) {
    ...
    if (data.post_title == "" || data.post_type_id == "") {
      showAlert(
        "warning",
        "<p>Không Được Để Trống Tiêu Đề Hoặc Nhóm Bài Viết</p>"
      );
    } else {
      if (data.content == "") {
...
function formatRepoPostType(repo) {
  ...
  var $container = $(
    "<div class='select2-result-posttype clearfix' id='result_posttype_" +
      repo.id +
      "'>" +
...
function formatRepoSelectionPostType(state) {
  ...
  var $state = $('<span id = "posttype_' + state.id + '"><span></span></span>');
...
function formatRepoProduct(repo) {
  ...
  var $container = $(
    "<div class='select2-result-product clearfix' id='result_product_" +
      repo.id +
      "'>" +
      "<div class='select2-result-product__icon'><img src='" +
      base_url +
      repo.image +
      "' /></div>" +
  ...
  $container
    .find(".select2-result-product__description")
    .text(repo.description);
